...
if(isset($_GET['id']))
{
	...
	@$sql="SELECT * FROM `tblfluxo`, `paramtipos` WHERE `paramtipos`.`idTipo` LIKE `tblfluxo`.`idTipo` AND `tblfluxo`.`idTipoFluxo`=2 AND `tblfluxo`.`idfluxo`='$saidaId'";
...
{   
    ...
    @$stmtGM ="UPDATE `tblfluxo` SET `descricaofluxo`='$descricaoEntrada',`dataPrevista`='$dataPrevista',`valorPrevisto` = '$valorPrevisto',`idTipo`='$Tipo'  WHERE `tblfluxo`.`idfluxo` = '$saidaId'";
...
?>
<form action="editarSaidas.php?id=<?=$saidaId?>" method="post" name="form1" id="form1">
