<?php 
$sqlEntradaM = "SELECT MIN(idfluxo) as minimo, MAX(idfluxo) as maximo FROM `tblfluxo`";
...
<?php
for($i=$min; $i<=$max; $i++){
	$sqlEntrada = "SELECT *, DAY(STR_TO_DATE(dataPrevista, '%Y-%m-%d')) AS data FROM `tblfluxo` AS data WHERE idfluxo='$i'";
	...
	?>
		<tr>
			...
			<td><?=$rowEntrada['descricaofluxo']."/".$rowSaidas['descricaoSaida']?></td>
