...
$sqlTipo = "SELECT * FROM `tblfluxo`, `paramtipos` WHERE `paramtipos`.`idTipo` LIKE `tblfluxo`.`idTipo` AND `tblfluxo`.`idTipoFluxo`=2 ORDER BY `tblfluxo`.`idfluxo` DESC";
...
<table name="tabela" id="tabela" st-table="rowCollectionBasic" class="table m-b-none">
      ...
      <tr>
        ...
        <th st-sort="lastName" style="width: 28%">Descrição</th>
		<th st-sort="lastName">Data de previsão</th>
		<th st-sort="lastName">Previsão</th>
...
?>
  <tr>	
		...
		<td align="center">
			<a href="../display/editarSaidas.php?id=<?=$linha['idfluxo']?>"><button type="button" class="btn btn-sm btn-sm "  data-toggle="tooltip" title="Editar" <?=$Butao?> <?=$disableButons?>><i class="glyphicon glyphicon-edit"></i></button>
			</a>
			<a href="../display/executarSaidas.php?id=<?=$linha['idfluxo']?>"><button class="btn btn-sm btn-sm blue-700" data-toggle="tooltip" title="Executar" <?=$Butao?> <?=$disableButons?>><i class="glyphicon glyphicon-check"></i></button>
			</a>
			<!--<a href="#"><button class="btn btn-sm btn-sm " data-toggle="tooltip" title="Detalhes" <?=$disableButons?>><i class="glyphicon glyphicon-zoom-in"></i></button>
			</a>-->
