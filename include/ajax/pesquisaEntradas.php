...
//Consulta
$sql="SELECT * FROM `tblfluxo`, `paramtipos`, `tblempresas` WHERE `paramtipos`.`idTipo` LIKE `tblfluxo`.`idTipo` AND `tblfluxo`.`idEmpresa` LIKE `tblempresas`.`idEmpresa` ".$addSql." AND `tblfluxo`.`idTipoFluxo`=1 ORDER BY `tblfluxo`.`descricaofluxo` DESC";
$stmt=mysqli_query($con, $sql);

//Calculo do Total 
@$sqlTotal="SELECT SUM(valorPrevisto) as Total FROM `tblfluxo`, `paramtipos`, `tblempresas` WHERE `paramtipos`.`idTipo` LIKE `tblfluxo`.`idTipo` AND `tblfluxo`.`idEmpresa` LIKE `tblempresas`.`idEmpresa` AND `tblfluxo`.`idTipoFluxo`=1 ".$addSql;
...
<tr>				
	...
	<td><a href="../display/editarEntradas.php?id=<?=$linha['idfluxo']?>"><button type="button" class="btn btn-sm btn-sm "  data-toggle="tooltip" title="Editar" <?=$Butao?>><i class="glyphicon glyphicon-edit"></i></button></a>
 ...
 <a><button class="btn btn-sm btn-sm red-700" <?=$Butao?> data-toggle="modal" data-target="#modal<?php echo $linha['idfluxo'];?>" data-toggle="tooltip" title="Excluir"><i class="glyphicon glyphicon-remove"></i></button></a></td>
	</tr>
	<!-- .modal -->
